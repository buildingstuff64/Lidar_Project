...
def load_pcd(pcd_path):
    ...
    points = np.asarray(pcd.points)  #points and colours extracted + treated separately
...
def upsample_pcd_knn(points, colors, upsample_factor, knn_factor, cov_spread_factor):
    ...
    #o3d.visualization.draw_geometries([pcd])  #can comment if required, allows viewing of input PC

    pcd_tree = o3d.geometry.KDTreeFlann(pcd)  #calculates kNN tree of PC
...
def main():  #put absolute paths of files here, may need extra slahses
